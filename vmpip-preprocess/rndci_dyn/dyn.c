...
static int online_data_proc(uint8_t *data, uint32_t datalen)
{
	...
	while(movelen < datalen)
	{
		...
		{
			if((headerType = *(uint32_t *)chbuff) == TCPSERVER_HEADER_TYPE)
			{
				ndci_header_t ndci = *(ndci_header_t *)chbuff;
				if((ndci.len - chbufflen) <= (datalen - movelen))
				{
					memcpy(chbuff + chbufflen, movedata, (ndci.len - chbufflen));
					recv_call(chbuff, ndci.len);
					movelen += ndci.len - chbufflen;
					movedata = movedata + (ndci.len - chbufflen);
					chbufflen = 0;
				}
				else
				{
...
int online_init(uint32_t channel, int thr_id)
{
	...
	serv_param.listen_port_arr[0] = 41409;
	serv_param.listen_num         = 1;
